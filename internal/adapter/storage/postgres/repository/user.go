...
func (ur *UserRepository) CreateUser(ctx context.Context, user *domain.User) (*domain.User, error) {
	query := ur.db.QueryBuilder.Insert("users").
		Columns("username", "password", "email", "role", "rank", "hire_date", "last_login", "status").
		Values(user.UserName, user.Password, user.Email, user.Role, user.Rank, user.HireDate, user.LastLogin, user.Status).
		Suffix("RETURNING *")
